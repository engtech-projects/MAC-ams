            ...
            <th class="text-center">
              <div class="form-group">
                <div class="form-check">
                  <input class="form-check-input pr-chkbox-payment" type="checkbox" checked>
                </div>
              </div>
            </th>
            <th>Description</th>
            ...
            @foreach ($invoices as $invoice)
              <tr>
                <td class="text-center">
                  ...
                  <div class="form-group">
                    <div class="form-check">
                      <input class="form-check-input child-chkbox-payment" type="checkbox" data-balance="{{ $invoice->balance }}" checked>
                    </div>
                  </div>
                 
                </td>
                <td><span class="text-blue" > Invoice {{ $invoice->no }}</span> <span>{{ $invoice->due_date }}</span></td>
                <td>{{ $invoice->due_date }}</td>
                <td class="text-right">{{ $invoice->total_amount }}</td>
                <td class="text-right">{{ $invoice->balance }}</td>
                <td class="text-right">
                  <input type="hidden" value="{{ $invoice->invoice_id }}" name="reference_id" form="frm-payment">
                  <input type="text" class="form-control form-control-sm rounded-0 text-right txt-payment" form="frm-payment" name="amount" value="{{ $invoice->balance }}">
                </td>
              </tr>
            @endforeach
          <!-- <tr>
            ...
            <td class="text-right invoice-balance"></td>
            <td class="text-right">
              <input type="hidden" value="" name="reference_id" form="frm-payment">
              <input type="text" class="form-control form-control-sm rounded-0 text-right txt-payment" form="frm-payment" name="amount" value="">
            </td>
          </tr> -->
