...
<section class="content">
  <div class="container-fluid" style="padding:32px;background-color:#fff;min-height:900px;">
	<form action="{{route('product.store')}}" method="post" enctype="multipart/form-data">
		...
		<div class="row">
			<div class="col-md-8">
				<h3 style="margin-bottom:32px;">Product Information</h3>
				<div class="row">
					<div class="col-md-6">
							...
							<div style="display:flex;flex-direction:column;flex:1;margin-right:32px;">
								<div class="form-group">
									...
									<div class="input-group">
										<input name="name" id="name" value="{{isset($product->name)? $product->name : old('name')}}" class="form-control form-control-sm rounded-0" required>
									</div>
								</div>
								<div class="form-group">
									<label class="label-normal" for="exampleInputPassword1">SKU</label>
									<div class="input-group">
										<input name="sku" id="sku" value="{{isset($product->sku)? $product->sku : old('sku')}}" class="form-control form-control-sm rounded-0">
									</div>
								</div>
							</div>

							<div style="display:flex;flex-direction:column">
								<img id="productImage" src="{{!empty($product->image_path)? asset($product->image_path) : asset('img/img_temp.png')}}" alt="" style="width:105px;height:auto;margin-bottom:5px;cursor:pointer;">
						...
						<div>
							<div class="form-group">
								...
								<div class="input-group">
									<select name="category" id="categoriesSelect" class="form-control form-control-sm rounded-0 mr-3">
										@foreach($categories as $category)
										<option value="{{$category->ps_category_id}}" {{isset($category->ps_category_id)? selected($category->ps_category_id, $product->ps_category_id, 'selected') : ''}}>{{$category->category_name}}</option>
										@endforeach
						...
						<div style="display:flex;flex-direction:column;margin-bottom:16px;padding:24px;background-color:#f2f2f2;">
							<div style="display:flex;margin-bottom:16px;">
								...
								<input type="number" style="flex:1" name="q_onhand" id="qOnhand" value="{{isset($product->qty_on_hand)? $product->qty_on_hand : old('q_onhand')}}" class="form-control form-control-sm rounded-0">
							</div>
							<div style="display:flex;margin-bottom:16px;">
								<span style="flex:2">As of date</span>
								<input style="flex:1" type="date" name="asofdate" id="asOfDate" value="{{old('asofdate')}}" class="form-control form-control-sm rounded-0">
							</div>
							<div style="display:flex;">
								<span style="flex:2">Reorder point</span>
								<input type="number" style="flex:1" name="reorder_point" id="reorderPoint" value="{{isset($product->reorder_point)? $product->reorder_point : old('reorder_point')}}" class="form-control form-control-sm rounded-0">
							</div>
						</div>
					...
					<div class="col-md-6">
						<div style="display:flex;">
							<div class="form-group" style="flex:1">   
								...
								<div class="input-group">
									<textarea class="form-control form-control-sm rounded-0" name="description" id="description" cols="30" rows="3" placeholder="Description on sales forms" value="{{old('item_description')}}">{{isset($product->description)? $product->description : old('description')}}</textarea>
								</div>
							</div>
						</div>
						<div style="display:flex;">
							<div class="form-group" style="margin-right:10px;flex:1;">
								...
								<div class="input-group">
									<input type="number" name="sales_price_rate" value="{{isset($product->rate)? $product->rate : old('sales_price_rate')}}" class="form-control form-control-sm rounded-0" id="salesPriceRate" placeholder="">
								</div>
							</div>
							<div class="form-group" style="margin-right:10px;flex:1;">
						...
						<div style="display:flex;">
							<div class="form-group" style="flex:1">   
								...
								<div class="input-group">
									<textarea class="form-control form-control-sm rounded-0" name="purchase_info" id="purchaseinfo" cols="30" rows="3" placeholder="Description on purchase forms" value="{{old('item_description')}}"></textarea>
								</div>
							</div>
						</div>
						...
						<div style="display:flex;">
							<div class="form-group" style="margin-right:10px;flex:1;">
								...
								<div class="input-group">
									<input type="number" name="cost" value="{{old('cost')}}" class="form-control form-control-sm rounded-0" id="costValue" placeholder="">
								</div>
							</div>
							<div class="form-group" style="margin-right:10px;flex:1;">
