...
<section class="content">
  <div class="container-fluid" style="padding:32px;background-color:#fff;min-height:900px;">
	<form action="{{route('service.store')}}" method="post" enctype="multipart/form-data">
		...
		<div class="row">
			<div class="col-md-8">
				<h3 style="margin-bottom:32px;">Service Information</h3>
				<div class="row">
					<div class="col-md-6">
							...
							<div style="display:flex;flex-direction:column;flex:1;margin-right:32px;">
								<div class="form-group">
									...
									<div class="input-group">
										<input name="name" id="name" value="{{isset($service->name)? $service->name : old('name')}}" class="form-control form-control-sm rounded-0" required>
									</div>
								</div>
								<div class="form-group">
									<label class="label-normal" for="exampleInputPassword1">SKU</label>
									<div class="input-group">
										<input name="sku" id="sku" value="{{isset($service->sku)? $service->sku : old('sku')}}" class="form-control form-control-sm rounded-0">
									</div>
								</div>
							</div>

							<div style="display:flex;flex-direction:column">
								<img id="productImage" src="{{!empty($service->image_path)? asset($service->image_path) : asset('img/img_temp.png')}}" alt="" style="width:105px;height:auto;margin-bottom:5px;cursor:pointer;">
							...
							<div class="form-group">
								...
								<div class="input-group">
									<select name="category" id="category" class="form-control form-control-sm rounded-0 mr-2">
										...
										<option value="{{$category->ps_category_id}}" {{isset($category->ps_category_id)? selected($category->ps_category_id, $service->ps_category_id, 'selected') : ''}}>{{$category->category_name}}</option>
										@endforeach
									</select>
					...
					<div class="col-md-6">
						<div style="display:flex;">
							<div class="form-group" style="flex:1">   
								...
								<div class="input-group">
									<textarea class="form-control form-control-sm rounded-0" style="height:107px" name="description" id="description" cols="30" rows="4" placeholder="Description on sales forms" value="{{old('item_description')}}">{{isset($service->description)? $service->description : ''}}</textarea>
								</div>
							</div>
						</div>
						<div style="display:flex;">
							<div class="form-group" style="margin-right:10px;flex:1;">
								...
								<div class="input-group">
									<input type="number" name="sales_price_rate" value="{{isset($service->rate)? $service->rate : old('sales_price_rate')}}" class="form-control form-control-sm rounded-0" id="salesPriceRate" placeholder="">
								</div>
							</div>
							<div class="form-group" style="margin-right:10px;flex:1;">
